...
таблицу logs - TODO
- Используется триггер `AFTER UPDATE` на таблицу users - TODO
- Пример: изменение `email → logs` получает `user_id`, `action`, `timestamp` - TODO
---
### SQL-функции
- `get_user_by_id(id)` — возвращает пользователя - DONE
- `recent_logs(user_id)` — последние 5 действий - TODO
- `search_users(name_pattern)` — поиск по части имени - TODO
- `create_user(...)` — используется при регистрации - DONE
- Все функции вызываются из Fastify-роутов через параметризированные запросы - TODO
---
### Управление правами
- В БД создать двух пользователей:
  - `admin_user` — доступ ко всем операциям - TODO
  - `readonly_user` — только `SELECT` - TODO
- Попробовать подключиться под обоими и сравнить поведение - TODO
- Настроить `GRANT`, `REVOKE`, роли, схемы - TODO
---
...
- Реализовать поиск пользователя по имени `/api/search?name=...` - TODO
- Написать уязвимую версию (для демонстрации) - TODO
- Затем — исправить с параметризацией - TODO
